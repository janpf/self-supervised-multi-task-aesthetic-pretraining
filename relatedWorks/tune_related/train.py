...
from relatedWorks.tune_related.dataset import AVA
from relatedWorks.tune_related.NIMA import NIMA, earth_movers_distance
...
# misc
parser.add_argument("--log_dir", type=str, default="/scratch/train_logs/rel2NIMA/AVA")
parser.add_argument("--ckpt_path", type=str, default="/scratch/ckpts/rel2NIMA/AVA")
...
if config.load_path is not None:
    if "SimClR" in config.load_path:
        settings.append("SimClR")
        if "imgnet" in config.load_path:
            settings.append("imgnet")
        elif "random" in config.load_path:
            settings.append("random")
        else:
            raise "??"
        if "Adam" in config.load_path:
            settings.append("adam")
        elif "LARS" in config.load_path:
            settings.append("lars")
        else:
            print("nothing, so likely adam??")
            settings.append("adam")
    elif "rotnet" in config.load_path:
        settings.append("rotnet")
        if "ImageNet" in config.load_path:
            settings.append("imgnet")
        elif "Random" in config.load_path:
            settings.append("random")
        else:
